...
**Rooms.** All 128 of the original's rooms, generated by `tools/rip/castle.py` from its room table (decoded by `tools/rip/rooms.py`, following the loader at 0xD3C6) into `src/scenes/rooms/roomSpecs.ts`, built by `specBuilder.ts`:
- **Doors** come from the table's background pieces.
